...
The system consists of a backend (Python) that converts PDFs, stores them as searchable data, and runs queries; a database (Qdrant) that holds the indexed content; and a web interface hosted at [https://mine-dd.github.io/PRAG-AI](https://mine-dd.github.io/PRAG-AI). The backend and database run locally inside Docker. The web interface connects to them over your local network.
...
You can verify everything is running at [http://localhost:8000/health](http://localhost:8000/health) — you should see a short status message in your browser.
...
In the Settings panel, scroll to **Ollama Models** and use the **Pull model** section. Select a model from the dropdown and click **Pull** to download it:
